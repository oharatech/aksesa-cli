...
from aksesa_cli.utils.editor import (
    edit_text_in_editor,
    get_editor_command,
)
...
class TestGetEditorCommand:
    ...
    def test_auto_detect_order(self, monkeypatch: pytest.MonkeyPatch):
        ...
        with patch("aksesa_cli.utils.editor.shutil.which", side_effect=fake_which):
            assert get_editor_command() == ["vim"]
    ...
    def test_returns_none_when_nothing_available(self, monkeypatch: pytest.MonkeyPatch):
        ...
        with patch("aksesa_cli.utils.editor.shutil.which", return_value=None):
...
@pytest.mark.skipif(os.name == "nt", reason="Editor tests rely on POSIX shell scripts")
class TestEditTextInEditor:
    ...
    def test_subprocess_call_uses_clean_env(self, tmp_path: Path):
        ...
        with patch("aksesa_cli.utils.editor.subprocess.call", return_value=0) as mock_call:
    ...
    def test_temp_file_has_md_suffix(self, tmp_path: Path):
        ...
        editor = self._make_fake_editor(tmp_path, modify=True)
        with patch("aksesa_cli.utils.editor.subprocess.call", side_effect=spy_call):
            edit_text_in_editor("text", configured=editor)
...
class TestConfigIntegration:
    ...
    def test_default_editor_empty_by_default(self):
        ...
        from aksesa_cli.config import get_default_config
    ...
    def test_load_config_with_editor(self):
        ...
        from aksesa_cli.config import load_config_from_string
