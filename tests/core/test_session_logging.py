...
from aksesa_cli.cli.export import _collect_recent_log_files, _session_time_range
...
class TestCollectRecentLogFiles:
    ...
    def test_collects_logs_near_export_time(self, tmp_path: Path):
        ...
        with patch("aksesa_cli.share.get_share_dir", return_value=tmp_path):
...
class TestToolExecutionLogging:
    ...
    async def test_toolset_tool_execution_error_logged(self):
        ...
        from aksesa_cli.wire.types import ToolCall
        ...
        with patch("aksesa_cli.soul.toolset.logger") as mock_logger:
...
class TestFileToolLogging:
    ...
    async def test_read_file_exception_logged(self, read_file_tool):
        from aksesa_cli.tools.file.read import Params

        with (
            patch("aksesa_cli.tools.file.read.logger") as mock_logger,
            patch("aksesa_cli.tools.file.read.kaos_path_from_user_input") as mock_path,
        ):
    ...
    async def test_write_file_exception_logged(self, write_file_tool):
        from aksesa_cli.tools.file.write import Params

        with (
            patch("aksesa_cli.tools.file.write.logger") as mock_logger,
            patch("aksesa_cli.tools.file.write.kaos_path_from_user_input") as mock_path,
        ):
            mock_path.side_effect = RuntimeError("Unexpected")
    ...
    async def test_glob_exception_logged(self, glob_tool):
        from aksesa_cli.tools.file.glob import Params

        with (
            patch("aksesa_cli.tools.file.glob.logger") as mock_logger,
            patch("aksesa_cli.tools.file.glob.kaos_path_from_user_input") as mock_path,
        ):
            mock_path.side_effect = RuntimeError("Unexpected")
    ...
    async def test_replace_file_exception_logged(self, str_replace_file_tool):
        from aksesa_cli.tools.file.replace import Edit, Params

        with (
            patch("aksesa_cli.tools.file.replace.logger") as mock_logger,
            patch("aksesa_cli.tools.file.replace.kaos_path_from_user_input") as mock_path,
        ):
            mock_path.side_effect = RuntimeError("Unexpected")
...
class TestSearchWebLogging:
    async def test_search_timeout_logged(self, search_web_tool):
        from aksesa_cli.tools.web.search import Params
        from tests.conftest import tool_call_context

        with (
            tool_call_context("SearchWeb"),
            patch("aksesa_cli.tools.web.search.logger") as mock_logger,
            patch("aksesa_cli.tools.web.search.new_client_session") as mock_session,
        ):
...
class TestLLMLogging:
    def test_create_llm_missing_config_logged(self):
        from aksesa_cli.config import LLMModel, LLMProvider

        with patch("aksesa_cli.llm.logger") as mock_logger:
            from aksesa_cli.llm import create_llm
...
class TestSubagentRunnerLogging:
    async def test_max_steps_reached_logged(self):
        from aksesa_cli.soul import MaxStepsReached
        from aksesa_cli.subagents.runner import run_soul_checked

        with (
            patch("aksesa_cli.subagents.runner.run_soul") as mock_run_soul,
            patch("aksesa_cli.subagents.runner.logger") as mock_logger,
        ):
