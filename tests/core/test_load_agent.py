...
from aksesa_cli.exception import InvalidToolError, SystemPromptTemplateError
from aksesa_cli.session import Session
from aksesa_cli.soul.agent import BuiltinSystemPromptArgs, Runtime, _load_system_prompt, load_agent
...
def test_load_tools_valid(runtime: Runtime):
    ...
    tool_paths = ["aksesa_cli.tools.think:Think", "aksesa_cli.tools.shell:Shell"]
    toolset = KimiToolset()
    toolset.load_tools(
        tool_paths,
        {
...
def test_load_tools_invalid(runtime: Runtime):
    ...
    tool_paths = ["aksesa_cli.tools.nonexistent:Tool", "aksesa_cli.tools.think:Think"]
    toolset = KimiToolset()
    try:
        toolset.load_tools(
            tool_paths,
        ...
        assert "aksesa_cli.tools.nonexistent:Tool" in str(e)
...
agent:
  name: "Test Agent"
  system_prompt_path: ./system.md
  tools: ["aksesa_cli.tools.nonexistent:Tool"]
""")
