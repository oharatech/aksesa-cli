...
from aksesa_cli.cli import ExitCode, InputFormat
from aksesa_cli.soul import RunCancelled
...
async def test_print_reruns_soul_on_pending_notification(tmp_path: Path) -> None:
    ...
    with patch("aksesa_cli.ui.print.run_soul", side_effect=fake_run_soul):
...
async def test_print_calls_reconcile_each_poll_iteration(tmp_path: Path) -> None:
    ...
    with (
        patch("aksesa_cli.ui.print.run_soul", side_effect=fake_run_soul),
        patch("aksesa_cli.ui.print.asyncio.sleep", side_effect=fake_sleep),
    ):
...
async def test_print_background_wait_cancel_returns_failure(tmp_path: Path) -> None:
    ...
    with (
        patch("aksesa_cli.ui.print.run_soul", side_effect=fake_run_soul),
        patch("aksesa_cli.ui.print.install_sigint_handler") as mock_sigint,
    ):
        cancel_handler = None
...
async def test_print_wait_timeout_kills_and_reenters_soul(tmp_path: Path) -> None:
    ...
    with (
        patch("aksesa_cli.ui.print.run_soul", side_effect=fake_run_soul),
        patch("aksesa_cli.ui.print.asyncio.sleep", side_effect=fake_sleep),
        patch("aksesa_cli.ui.print.time.monotonic", side_effect=fake_monotonic),
    ):
...
async def test_timeout_finally_reconcile_failure_preserves_run_cancelled(
    ...
    with (
        ...
        patch("aksesa_cli.ui.print.asyncio.sleep", side_effect=fake_sleep),
        patch("aksesa_cli.ui.print.time.monotonic", side_effect=fake_monotonic),
        patch("aksesa_cli.ui.print.logger") as mock_logger,
    ):
...
async def test_print_wait_timeout_writes_to_original_stderr_when_redirected(
    ...
    monkeypatch.setattr("aksesa_cli.ui.print.open_original_stderr", fake_open_original_stderr)
