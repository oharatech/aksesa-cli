...
from aksesa_cli.tools.plan import ExitPlanMode, Params, PlanOption
from aksesa_cli.tools.plan.enter import _DESCRIPTION, EnterPlanMode
from aksesa_cli.tools.plan.heroes import (
    _slug_cache,
    get_or_create_slug,
    get_plan_file_path,
    read_plan_file,
...
from aksesa_cli.wire.types import QuestionNotSupported, QuestionRequest, ToolCall
...
class TestGetOrCreateSlug:
    def test_returns_hero_name(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
        monkeypatch.setattr("aksesa_cli.tools.plan.heroes.PLANS_DIR", tmp_path)
    ...
    def test_collision_fallback(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        monkeypatch.setattr("aksesa_cli.tools.plan.heroes.HERO_NAMES", ["a", "b"])
...
def _mock_wire_and_tool_call(monkeypatch: pytest.MonkeyPatch):
    ...
    monkeypatch.setattr("aksesa_cli.tools.plan.get_wire_or_none", lambda: wire_mock)
    monkeypatch.setattr("aksesa_cli.tools.plan.wire_send", lambda msg: None)
    monkeypatch.setattr("aksesa_cli.tools.plan.enter.get_wire_or_none", lambda: wire_mock)
    monkeypatch.setattr("aksesa_cli.tools.plan.enter.wire_send", lambda msg: None)
    tc = ToolCall(id="test-tc", function=ToolCall.FunctionBody(name="ExitPlanMode", arguments=None))
    monkeypatch.setattr("aksesa_cli.tools.plan.get_current_tool_call_or_none", lambda: tc)
    monkeypatch.setattr("aksesa_cli.tools.plan.enter.get_current_tool_call_or_none", lambda: tc)
    return wire_mock
...
class TestExitPlanModeHappyPaths:
    ...
    async def test_wire_unavailable(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
        tool, _, _ = _setup_exit_tool(tmp_path)
        monkeypatch.setattr("aksesa_cli.tools.plan.get_wire_or_none", lambda: None)
...
class TestEnterPlanModeHappyPaths:
    ...
    async def test_wire_unavailable(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
        tool, _, _ = _setup_enter_tool(tmp_path)
        monkeypatch.setattr("aksesa_cli.tools.plan.enter.get_wire_or_none", lambda: None)
...
class TestKimiSoulPlanSessionPersistence:
    ...
    async def test_plan_session_id_cleared_after_deactivation(
        ...
        # Restart + re-activate
        from aksesa_cli.tools.plan.heroes import _slug_cache
