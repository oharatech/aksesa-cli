...
import aksesa_cli.app as app_module
...
class TestWebWorkerResumedDetection:
    ...
    async def test_new_session_without_state_file_is_not_resumed(self, tmp_path):
        ...
        fake_joint = SimpleNamespace(aksesa_cli_session=fake_session)

        with (
            patch("aksesa_cli.web.runner.worker.load_session_by_id", return_value=fake_joint),
            patch(
                "aksesa_cli.web.runner.worker.get_global_mcp_config_file",
