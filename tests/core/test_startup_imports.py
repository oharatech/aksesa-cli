...
def test_import_aksesa_cli_does_not_import_loguru() -> None:
    proc = _run_python(
        """
import sys
...
def test_import_aksesa_cli_constant_defers_package_metadata() -> None:
    proc = _run_python(
        """
import sys
sys.modules.pop("importlib.metadata", None)
import aksesa_cli.constant as constant
...
from aksesa_cli.cli import cli
...
assert result.exit_code == 0, result.output
assert '"aksesa_cli_version"' in result.output
assert "aksesa_cli.cli.info" in sys.modules
assert "aksesa_cli.cli.export" not in sys.modules
assert "aksesa_cli.cli.mcp" not in sys.modules
assert "aksesa_cli.cli.vis" not in sys.modules
assert "aksesa_cli.cli.web" not in sys.modules
print("ok")
"""
    )
    assert proc.stdout.strip() == "ok"
...
sys.modules.pop("aksesa_cli.cli", None)

from aksesa_cli.__main__ import main
...
assert exit_code == 0
assert stdout.getvalue().startswith("aksesa, version ")
assert "aksesa_cli.cli" not in sys.modules
print("ok")
"""
    )
    assert proc.stdout.strip() == "ok"
...
assert "aksesa_cli.cli" in sys.modules
payload = json.loads(stdout.getvalue())
assert payload["aksesa_cli_version"]
print("ok")
"""
    )
    assert proc.stdout.strip() == "ok"
...
import aksesa_cli.cli.__main__ as cli_main
from aksesa_cli.utils.environment import GitBashNotFoundError
