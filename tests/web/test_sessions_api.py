...
from aksesa_cli.web.api import sessions as sessions_api
from aksesa_cli.web.models import GenerateTitleRequest
...
async def test_generate_title_preserves_concurrent_manual_title(
    ...
    monkeypatch.setattr("aksesa_cli.config.load_config", lambda: config)
    monkeypatch.setattr(
        "aksesa_cli.llm.create_llm",
        lambda provider_config, model_config, oauth=None: _FakeLLM(),
    )
    monkeypatch.setattr("aksesa_cli.auth.oauth.OAuthManager", _FakeOAuthManager)
