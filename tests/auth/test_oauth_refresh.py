...
from aksesa_cli.auth.oauth import (
...
from aksesa_cli.config import Config, LLMModel, LLMProvider, OAuthRef, Services
...
def _make_manager(token: OAuthToken | None = None) -> OAuthManager:
    with patch("aksesa_cli.auth.oauth.load_tokens", return_value=token):
...
async def test_refresh_token_retries_on_network_error():
    ...
    with patch("aksesa_cli.auth.oauth.new_client_session", return_value=FakeSession()):
...
async def test_refresh_token_does_not_retry_on_unauthorized():
    ...
    with (
        patch("aksesa_cli.auth.oauth.new_client_session", return_value=FakeSession()),
...
async def test_ensure_fresh_force_bypasses_threshold():
    ...
    with (
        ...
        patch("aksesa_cli.auth.oauth.refresh_token", mock_refresh),
        patch("aksesa_cli.auth.oauth.save_tokens"),
    ):
...
def test_save_to_file_is_atomic(tmp_path):
    ...
    key = "test-atomic"
    with patch("aksesa_cli.auth.oauth._credentials_dir", return_value=tmp_path):
        token = _make_token()
...
async def test_rejected_refresh_token_cooldown_skips_background_retry(tmp_path, monkeypatch):
    ...
    with (
        patch("aksesa_cli.auth.oauth.refresh_token", refresh),
        patch("aksesa_cli.auth.oauth.asyncio.sleep", new=AsyncMock()),
    ):
...
async def test_ensure_fresh_non_force_swallows_errors():
    ...
    with (
        patch("aksesa_cli.auth.oauth.load_tokens", return_value=token),
        patch("aksesa_cli.auth.oauth.refresh_token", AsyncMock(side_effect=OAuthError("fail"))),
    ):
