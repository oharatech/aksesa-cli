...
def _make_oauth_manager(config: Config, initial_token: OAuthToken | None = None) -> OAuthManager:
    ...
    with patch("aksesa_cli.auth.oauth.load_tokens", return_value=initial_token):
...
async def test_ensure_fresh_without_runtime_caches_token():
    ...
    with patch("aksesa_cli.auth.oauth.load_tokens", return_value=fresh_token):
...
async def test_ensure_fresh_without_runtime_refreshes_expired_token():
    ...
    with (
        patch("aksesa_cli.auth.oauth.load_tokens", return_value=expired_token),
        patch(
            "aksesa_cli.auth.oauth.refresh_token",
