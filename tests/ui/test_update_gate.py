...
from aksesa_cli.ui.shell.update import semver_tuple
...
class TestCheckUpdateGate:
    ...
    def _setup(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("aksesa_cli.ui.shell.update.LATEST_VERSION_FILE", self.latest_file)
        monkeypatch.setattr("aksesa_cli.ui.shell.update.SKIPPED_VERSION_FILE", self.skipped_file)
        monkeypatch.setattr("aksesa_cli.constant.VERSION", "1.2.3")
        ...
        monkeypatch.setattr("aksesa_cli.ui.shell.update._run_update_gate", self._run_gate_mock)

    def test_skips_when_auto_update_disabled(self, monkeypatch):
        from aksesa_cli.ui.shell.update import check_update_gate

        self.latest_file.write_text("2.0.0")
        monkeypatch.setenv("AKSESA_CLI_NO_AUTO_UPDATE", "1")
        check_update_gate()
    ...
    def test_skips_when_latest_file_unreadable(self, monkeypatch):
        ...
        self.latest_file.write_text("2.0.0")
        monkeypatch.setattr(
            "aksesa_cli.ui.shell.update.LATEST_VERSION_FILE",
            MagicMock(
    ...
    def test_triggers_when_skipped_file_unreadable(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "aksesa_cli.ui.shell.update.SKIPPED_VERSION_FILE",
            MagicMock(
                exists=MagicMock(return_value=True), read_text=MagicMock(side_effect=OSError)
            ),
        )
...
class TestRunUpdateGate:
    ...
    def _setup(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("aksesa_cli.ui.shell.update.SKIPPED_VERSION_FILE", self.skipped_file)
        # Silence rich output
        monkeypatch.setattr("aksesa_cli.ui.shell.update.console", MagicMock())

    def test_enter_cr_runs_upgrade_and_exits_zero(self, monkeypatch):
        from aksesa_cli.ui.shell.update import _run_update_gate

        monkeypatch.setattr("aksesa_cli.ui.shell.update._read_key", lambda: "\r")
    ...
    def test_enter_lf_runs_upgrade_and_exits_zero(self, monkeypatch):
        ...
        monkeypatch.setattr("aksesa_cli.ui.shell.update._read_key", lambda: "\n")
        mock_run = MagicMock(return_value=MagicMock(returncode=0))
        monkeypatch.setattr("subprocess.run", mock_run)
    ...
    def test_s_writes_skip_file_and_continues(self, monkeypatch):
        ...
        monkeypatch.setattr("aksesa_cli.ui.shell.update._read_key", lambda: "s")
        _run_update_gate("1.2.3", "1.5.0")
    ...
    def test_q_continues_without_action(self, monkeypatch):
        ...
        monkeypatch.setattr("aksesa_cli.ui.shell.update._read_key", lambda: "q")
        mock_run = MagicMock()
        monkeypatch.setattr("subprocess.run", mock_run)
        _run_update_gate("1.2.3", "1.5.0")
    ...
    def test_ctrl_c_exits_cleanly(self, monkeypatch):
        ...
        monkeypatch.setattr("aksesa_cli.ui.shell.update._read_key", lambda: "\x03")
        with pytest.raises(SystemExit) as exc_info:
            _run_update_gate("1.2.3", "1.5.0")
        assert exc_info.value.code == 0
    ...
    def test_esc_exits_cleanly(self, monkeypatch):
        ...
        monkeypatch.setattr("aksesa_cli.ui.shell.update._read_key", lambda: "\x1b")
        with pytest.raises(SystemExit) as exc_info:
            _run_update_gate("1.2.3", "1.5.0")
        assert exc_info.value.code == 0
    ...
    def test_unknown_key_continues(self, monkeypatch):
        ...
        monkeypatch.setattr("aksesa_cli.ui.shell.update._read_key", lambda: "x")
        mock_run = MagicMock()
        monkeypatch.setattr("subprocess.run", mock_run)
        _run_update_gate("1.2.3", "1.5.0")
        assert not self.skipped_file.exists()
...
class TestReadKeyWindows:
    def test_win32_uses_msvcrt(self, monkeypatch):
        ...
        from aksesa_cli.ui.shell.update import _read_key
...
class TestAutoUpdateNoToast:
    ...
    async def test_update_available_produces_no_toast(self, monkeypatch, mock_self):
        ...
        async def fake_do_update(*, print, check_only):
            from aksesa_cli.ui.shell.update import UpdateResult

            return UpdateResult.UPDATE_AVAILABLE

        monkeypatch.setattr("aksesa_cli.ui.shell.do_update", fake_do_update)
        mock_toast = MagicMock()
        monkeypatch.setattr("aksesa_cli.ui.shell.toast", mock_toast)
...
class TestPrintWelcomeInfoSkipsVersion:
    ...
    def _setup(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("aksesa_cli.ui.shell.LATEST_VERSION_FILE", self.latest_file)
        monkeypatch.setattr("aksesa_cli.ui.shell.update.SKIPPED_VERSION_FILE", self.skipped_file)
        monkeypatch.setattr("aksesa_cli.constant.VERSION", "1.2.3")
        monkeypatch.setattr("aksesa_cli.ui.shell.console", MagicMock())
    ...
    def test_welcome_shows_update_when_not_skipped(self, monkeypatch):
        from aksesa_cli.ui.shell import _print_welcome_info

        self.latest_file.write_text("2.0.0")
        mock_console = MagicMock()
        monkeypatch.setattr("aksesa_cli.ui.shell.console", mock_console)
    ...
    def test_welcome_no_crash_when_latest_file_unreadable(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "aksesa_cli.ui.shell.LATEST_VERSION_FILE",
            MagicMock(
                exists=MagicMock(return_value=True), read_text=MagicMock(side_effect=OSError)
            ),
        )
