...
def test_shell_smoke_multiturn_scripted_echo(tmp_path: Path) -> None:
    ...
    try:
        shell.read_until_contains("Welcome to Aksesa CLI!")
        ...
        json.dumps(
            {
                ...
                "providers": {
                    "root_provider": {
                        ...
                        "base_url": "",
                        "api_key": "",
                        "env": {"AKSESA_SCRIPTED_ECHO_SCRIPTS": str(root_scripts_path)},
                    },
                    "sub_provider": {
                        "type": "_scripted_echo",
                        "base_url": "",
                        "api_key": "",
                        "env": {"AKSESA_SCRIPTED_ECHO_SCRIPTS": str(sub_scripts_path)},
                    },
                },
            }
        ),
...
def test_shell_session_resume_and_replay(tmp_path: Path) -> None:
    ...
    try:
        first_shell.read_until_contains("Welcome to Aksesa CLI!")
    ...
    try:
        second_shell.read_until_contains("Welcome to Aksesa CLI!")
...
def test_shell_clear_reloads_without_replaying_old_turns(tmp_path: Path) -> None:
    ...
    try:
        ...
        shell.read_until_contains("Welcome to Aksesa CLI!", after=clear_mark)
