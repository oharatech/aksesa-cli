...
- Core：新增共享的 `file_filter` 模块——通过 `src/aksesa_cli/utils/file_filter.py` 统一 Shell 和 Web 的文件引用逻辑，提供一致的路径过滤、忽略目录排除和 Git 感知文件发现
...
- Lib：从 `aksesa_cli.wire.types` 重新导出所有 Wire 消息类型，作为 `aksesa_cli.wire.message` 的替代
...
- Lib：将 `MCPTool` 从 `aksesa_cli.tools.mcp` 移至 `aksesa_cli.soul.toolset`
...
- Lib：将 `WireMessage` 类型别名移至 `aksesa_cli.wire.message`
- Lib：添加 `aksesa_cli.wire.message.Request` 类型别名，用于请求消息（目前仅包含 `ApprovalRequest`）
- Lib：添加 `aksesa_cli.wire.message.is_event`、`is_request` 和 `is_wire_message` 工具函数，检查 Wire 消息类型
- Lib：添加 `aksesa_cli.wire.serde` 模块，用于 Wire 消息的序列化和反序列化
- Lib：修改 `StatusUpdate` Wire 消息，不再使用 `aksesa_cli.soul.StatusSnapshot`
...
- Tool：将 `Task` 工具移至 `aksesa_cli.tools.multiagent` 模块
...
- Lib：添加 `aksesa_cli.app.enable_logging` 函数，直接使用 `KimiCLI` 类时启用日志
...
- Lib：将 `WireMessage` 从 `aksesa_cli.wire.message` 移至 `aksesa_cli.wire`
...
- Lib：将 `LLMModelCapability` 重命名为 `ModelCapability` 并移至 `aksesa_cli.llm`
