...
The `CreateSubagent` and `Task` tools under `aksesa_cli.tools.multiagent` have been removed. Use the new `Agent` tool instead.

- **Affected**: Custom agent configurations referencing `aksesa_cli.tools.multiagent:Task` or `aksesa_cli.tools.multiagent:CreateSubagent`
- **Migration**: Replace with `aksesa_cli.tools.agent:Agent` in your agent YAML `allowed_tools`
...
The `Task` tool has been moved from `aksesa_cli.tools.task` to `aksesa_cli.tools.multiagent`.

- **Affected**: Custom tools importing the `Task` tool
- **Migration**: Change import path to `from aksesa_cli.tools.multiagent import Task`
