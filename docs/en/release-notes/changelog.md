...
- Core: Add shared `file_filter` module — unifies file mention logic between shell and web UIs via `src/aksesa_cli/utils/file_filter.py`, providing consistent path filtering, ignored directory exclusion, and git-aware file discovery
...
- Lib: Re-export all Wire message types from `aksesa_cli.wire.types`, as a replacement of `aksesa_cli.wire.message`
...
- Lib: Move `MCPTool` from `aksesa_cli.tools.mcp` to `aksesa_cli.soul.toolset`
...
- Lib: Move `WireMessage` type alias to `aksesa_cli.wire.message`
- Lib: Add `aksesa_cli.wire.message.Request` type alias request messages (which currently only includes `ApprovalRequest`)
- Lib: Add `aksesa_cli.wire.message.is_event`, `is_request` and `is_wire_message` utility functions to check the type of wire messages
- Lib: Add `aksesa_cli.wire.serde` module for serialization and deserialization of wire messages
- Lib: Change `StatusUpdate` Wire message to not using `aksesa_cli.soul.StatusSnapshot`
...
- Tool: Move `Task` tool to `aksesa_cli.tools.multiagent` module
...
- Lib: Add `aksesa_cli.app.enable_logging` function to enable logging when directly using `KimiCLI` class
...
- Lib: Move `WireMessage` from `aksesa_cli.wire.message` to `aksesa_cli.wire`
...
- Lib: Rename `LLMModelCapability` to `ModelCapability` and move to `aksesa_cli.llm`
