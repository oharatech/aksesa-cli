...
数据结构（位于 `src/aksesa_cli/skill/flow/__init__.py`，`PromptFlow` 更名为 `Flow`）：
...
**FlowRunner 类**（位于 `src/aksesa_cli/soul/kimisoul.py`）：
...
- CLI 入口：`src/aksesa_cli/cli/__init__.py`
- Skill 解析：`src/aksesa_cli/skill/__init__.py`
- Flow 解析：`src/aksesa_cli/skill/flow/mermaid.py` / `src/aksesa_cli/skill/flow/d2.py`
- Flow 数据结构：`src/aksesa_cli/skill/flow/__init__.py`
- `KimiSoul` 与 `FlowRunner`：`src/aksesa_cli/soul/kimisoul.py`
- `SlashCommand`：`src/aksesa_cli/utils/slashcmd.py`
- 静态 soul commands：`src/aksesa_cli/soul/slash.py`
- Shell UI：`src/aksesa_cli/ui/shell/__init__.py`
