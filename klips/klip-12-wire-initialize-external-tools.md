...
当前 Wire 协议（`docs/zh/customization/wire-mode.md` + `src/aksesa_cli/wire/*`,
`src/aksesa_cli/ui/wire/*`）只包含：
...
- `slash_commands` 仅包含 soul-level 命令：
  `src/aksesa_cli/soul/slash.py` registry + 动态 skills（`KimiSoul._register_skill_commands`）。
...
- Wire 协议与类型：`src/aksesa_cli/wire/types.py`, `src/aksesa_cli/wire/serde.py`
- Wire JSON-RPC：`src/aksesa_cli/ui/wire/jsonrpc.py`, `src/aksesa_cli/ui/wire/__init__.py`
- Slash commands：`src/aksesa_cli/soul/slash.py`, `src/aksesa_cli/utils/slashcmd.py`
