...
* `/setup` 位于 `src/aksesa_cli/ui/shell/setup.py`：选择平台、输入 API key、调用 `list_models(platform, api_key)` 获取并过滤模型，然后写入 `config.providers` 与 `config.models`，并设置 `default_model` 为用户选中的模型。
* `Config` 定义在 `src/aksesa_cli/config.py`：`providers` 与 `models` 平级，`default_model` 必须指向 `models` 中的键，且每个 `LLMModel.provider` 必须存在于 `providers`。
...
将 `/setup` 平台清单抽到公共模块（例如 `src/aksesa_cli/auth/platforms.py`），提供：
...
在 `/model` 命令（`src/aksesa_cli/ui/shell/slash.py`）触发刷新逻辑，仅在默认配置文件位置时启用：
...
* `/setup`：`src/aksesa_cli/ui/shell/setup.py`
* 配置结构：`src/aksesa_cli/config.py`
* 平台与模型刷新：`src/aksesa_cli/auth/platforms.py`
* `/model`：`src/aksesa_cli/ui/shell/slash.py`
