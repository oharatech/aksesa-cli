...
- 项目仓库与 PyPI 主包仍为 `kimi-cli`，Python 导入路径为 `aksesa_cli`。
...
- 保持命令与包名不变：`kimi` 命令、`kimi-cli` 包、`aksesa_cli` 导入路径继续使用。
...
- **Shell UI**：`src/aksesa_cli/ui/shell/*`
- **运行时品牌名称**：`src/aksesa_cli/constant.py`, `src/aksesa_cli/acp/server.py`,
  `src/aksesa_cli/cli/__init__.py`, `src/aksesa_cli/wire/server.py`
- **系统提示词**：`src/aksesa_cli/agents/default/system.md`
- **内置技能提示词**：`src/aksesa_cli/skills/*/SKILL.md`
...
- 与实际输出绑定的字段名/命令保持不变，例如 `aksesa_cli_version`、`uv tool upgrade kimi-cli`。
