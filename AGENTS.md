...
- **CLI entry**: `src/aksesa_cli/cli/__init__.py` (Typer) parses flags (UI mode, agent spec, config, MCP)
  and routes into `AksesaCLI` in `src/aksesa_cli/app.py`.
- **App/runtime setup**: `AksesaCLI.create` loads config (`src/aksesa_cli/config.py`), chooses a

  model/provider (`src/aksesa_cli/llm.py`), builds a `Runtime` (`src/aksesa_cli/soul/agent.py`),

  loads an agent spec, restores `Context`, then constructs `AksesaSoul`.
- **Agent specs**: YAML under `src/aksesa_cli/agents/` loaded by `src/aksesa_cli/agentspec.py`.
...
- **Tooling**: `src/aksesa_cli/soul/toolset.py` loads tools by import path, injects dependencies,
  and runs tool calls. Built-in tools live in `src/aksesa_cli/tools/` (agent, shell, file, web,
  todo, background, dmail, think, plan). MCP tools are loaded via `fastmcp`; CLI management is
  in `src/aksesa_cli/mcp.py` and stored in the share dir.
- **Subagents**: `LaborMarket` in `src/aksesa_cli/soul/agent.py` registers builtin subagent types.
  The `Agent` tool (`src/aksesa_cli/tools/agent/`) creates or resumes subagent instances, while
  `SubagentStore` persists instance metadata, prompts, wire logs, and context under
  `session/subagents/<agent_id>/`.
- **Core loop**: `src/aksesa_cli/soul/kimisoul.py` is the main agent loop. It accepts user input,

  handles slash commands (`src/aksesa_cli/soul/slash.py`), appends to `Context`

  (`src/aksesa_cli/soul/context.py`), calls the LLM (kosong), runs tools, and performs compaction

  (`src/aksesa_cli/soul/compaction.py`) when needed.
- **Approvals**: `src/aksesa_cli/soul/approval.py` is the tool-facing facade. `ApprovalRuntime`
  in `src/aksesa_cli/approval_runtime/` is the session-level source of truth for pending approvals,
  and approval requests are projected onto the root wire stream for Shell/Web style UIs.
- **UI/Wire**: `src/aksesa_cli/soul/run_soul` connects `KimiSoul` to a `Wire`
  (`src/aksesa_cli/wire/`) so UI loops can stream events. UIs live in `src/aksesa_cli/ui/`
  (shell/print/acp/wire).
- **Shell UI**: `src/aksesa_cli/ui/shell/` handles interactive TUI input, shell command mode,
  and slash command autocomplete; it is the default interactive experience.
- **Slash commands**: Soul-level commands live in `src/aksesa_cli/soul/slash.py`; shell-level
  commands live in `src/aksesa_cli/ui/shell/slash.py`. The shell UI exposes both and dispatches
...
- `src/aksesa_cli/app.py`: `KimiCLI.create(...)` and `KimiCLI.run(...)` are the main programmatic
  entrypoints; this is what UI layers use.
- `src/aksesa_cli/soul/agent.py`: `Runtime` (config, session, builtins), `Agent` (system prompt +
  toolset), and `LaborMarket` (builtin subagent type registry).
- `src/aksesa_cli/soul/kimisoul.py`: `KimiSoul.run(...)` is the loop boundary; it emits Wire
  messages and executes tools via `KimiToolset`.
- `src/aksesa_cli/soul/context.py`: conversation history + checkpoints; used by DMail for
  checkpointed replies.
- `src/aksesa_cli/soul/toolset.py`: load tools, run tool calls, bridge to MCP tools.
- `src/aksesa_cli/ui/*`: shell/print/acp frontends; they consume `Wire` messages.
- `src/aksesa_cli/wire/*`: event types and transport used between soul and UI.

## Repo map

- `src/aksesa_cli/agents/`: built-in agent YAML specs and prompts
- `src/aksesa_cli/prompts/`: shared prompt templates
- `src/aksesa_cli/soul/`: core runtime/loop, context, compaction, approvals
- `src/aksesa_cli/tools/`: built-in tools
- `src/aksesa_cli/ui/`: UI frontends (shell/print/acp/wire)
- `src/aksesa_cli/acp/`: ACP server components
...
- CLI entry point: `aksesa` -> `src/aksesa_cli/__main__.py` (routes to `src/aksesa_cli/cli/__init__.py`).
