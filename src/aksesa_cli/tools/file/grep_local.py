...
from aksesa_cli.utils.path import normalize_user_path
from aksesa_cli.utils.sensitive import is_sensitive_file, sensitive_file_warning
...
def _find_existing_rg(bin_name: str) -> Path | None:
    ...
    assert aksesa_cli.__file__ is not None
    local_dep = Path(aksesa_cli.__file__).parent / "deps" / "bin" / bin_name
