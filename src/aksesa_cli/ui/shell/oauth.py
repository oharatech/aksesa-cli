...
from aksesa_cli.ui.shell.setup import select_platform, setup_platform
from aksesa_cli.ui.shell.slash import ensure_kimi_soul, registry
...
async def _login_aksesa(soul: KimiSoul) -> bool:
    ...
    ok = True
    try:
        async for event in login_aksesa(soul.runtime.config):
            if event.type == "waiting":
                if status is None:
                    status = console.status("[cyan]Waiting for user authorization...[/cyan]")
                    status.start()
