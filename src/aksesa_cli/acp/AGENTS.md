...
- **Single-session server**: `KimiCLI.run_acp()` uses `ACP` -> `ACPServerSingleSession`.
  - Code: `src/aksesa_cli/app.py`, `src/aksesa_cli/ui/acp/__init__.py`.
  - Used when running CLI with `--acp` UI mode.
- **Multi-session server**: `acp_main()` runs `ACPServer` with `use_unstable_protocol=True`.
  - Code: `src/aksesa_cli/acp/__init__.py`, `src/aksesa_cli/acp/server.py`.
  - Exposed via the `kimi acp` command in `src/aksesa_cli/cli/__init__.py`.
