...
def kimi(
    ...
    from aksesa_cli.utils.proctitle import init_process_name
    ...
    from aksesa_cli.agentspec import DEFAULT_AGENT_FILE, OKABE_AGENT_FILE
    from aksesa_cli.app import KimiCLI, enable_logging
    from aksesa_cli.config import Config, load_config_from_string
    from aksesa_cli.exception import ConfigError
    from aksesa_cli.hooks import events as hook_events
    from aksesa_cli.metadata import load_metadata, save_metadata
    from aksesa_cli.session import Session
    from aksesa_cli.ui.shell.startup import ShellStartupProgress
    ...
    async def _run(session_id: str | None, prefill_text: str | None = None) -> tuple[Session, int]:
        ...
        try:
            ...
            if local_add_dirs:
                from aksesa_cli.utils.path import is_within_directory
    ...
    if _picker_mode:
        ...
        from aksesa_cli.utils.datetime import format_relative_time
    ...
    if switch_target in ("web", "vis"):
        from aksesa_cli.utils.logging import restore_stderr
        ...
        from aksesa_cli.utils.term import ensure_tty_sane

        ensure_tty_sane()

        if switch_target == "web":
            from aksesa_cli.web.app import run_web_server
...
def acp():
    ...
    from aksesa_cli.acp import acp_main
...
if __name__ == "__main__":
    ...
    if "aksesa_cli.cli" not in sys.modules:
        sys.modules["aksesa_cli.cli"] = sys.modules[__name__]
