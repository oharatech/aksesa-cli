...
from aksesa_cli.metadata import load_metadata, save_metadata
from aksesa_cli.session import Session as KimiCLISession
from aksesa_cli.utils.subprocess_env import get_clean_env
from aksesa_cli.web.auth import is_origin_allowed, is_private_ip, verify_token
from aksesa_cli.web.models import (
...
from aksesa_cli.web.runner.messages import new_session_status_message, send_history_complete
from aksesa_cli.web.runner.process import KimiCLIRunner
from aksesa_cli.web.store.sessions import (
    JointSession,
...
from aksesa_cli.wire.jsonrpc import (
    ErrorCodes,
    JSONRPCErrorObject,
...
from aksesa_cli.wire.serde import deserialize_wire_message
from aksesa_cli.wire.types import is_request
...
async def create_session(request: CreateSessionRequest | None = None) -> Session:
    ...
    aksesa_cli_session = await KimiCLISession.create(work_dir=work_dir)
    context_file = aksesa_cli_session.dir / "context.jsonl"
    invalidate_sessions_cache()
    invalidate_work_dirs_cache()
    return Session(
        session_id=UUID(aksesa_cli_session.id),
        title=aksesa_cli_session.title,
        ...
        session_dir=str(aksesa_cli_session.dir),
    )
...
async def upload_session_file(
    ...
    session_dir = session.aksesa_cli_session.dir
...
async def get_session_upload_file(
    ...
    uploads_dir = (session.aksesa_cli_session.dir / "uploads").resolve()
...
async def get_session_file(
    ...
    work_dir = Path(str(session.aksesa_cli_session.work_dir)).resolve()
...
def _update_last_session_id(session: JointSession) -> None:
    ...
    kimi_session = session.aksesa_cli_session
...
async def delete_session(session_id: UUID, runner: KimiCLIRunner = Depends(get_runner)) -> None:
    ...
    wd_meta = session.aksesa_cli_session.work_dir_meta
...
async def update_session(
    ...
    from aksesa_cli.session_state import load_session_state, save_session_state
...
async def fork_session_endpoint(
    ...
    """
    from aksesa_cli.session_fork import fork_session as do_fork

    source_session = get_editable_session(session_id, runner)
    source_dir = source_session.aksesa_cli_session.dir
    work_dir = source_session.aksesa_cli_session.work_dir
    ...
    from aksesa_cli.metadata import load_metadata
    from aksesa_cli.session_state import load_session_state
...
async def session_stream(
    ...
    try:
        ...
        try:
            ...
            work_dir = Path(str(session.aksesa_cli_session.work_dir))
