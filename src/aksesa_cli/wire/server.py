...
from aksesa_cli.wire import Wire
...
class WireServer:
    ...
    async def _handle_initialize(
        ...
        from aksesa_cli.hooks.config import HOOK_EVENT_TYPES
        from aksesa_cli.hooks.engine import WireHookHandle, WireHookSubscription
        from aksesa_cli.soul import wire_send
        from aksesa_cli.wire.protocol import WIRE_PROTOCOL_VERSION
        from aksesa_cli.wire.types import HookResolved, HookTriggered
    ...
    def _sync_ask_user_tool_visibility(self, toolset: KimiToolset) -> None:
        ...
        from aksesa_cli.tools.ask_user import NAME as ASK_USER_TOOL_NAME
    ...
    def _sync_plan_mode_tool_visibility(self, toolset: KimiToolset) -> None:
        ...
        from aksesa_cli.tools.plan import NAME as EXIT_PLAN_MODE_TOOL_NAME
        from aksesa_cli.tools.plan.enter import NAME as ENTER_PLAN_MODE_TOOL_NAME
    ...
    def _apply_wire_client_info(self, client: ClientInfo | None) -> None:
        if client is not None:
            from aksesa_cli.telemetry import set_client_info
        ...
        if isinstance(llm.chat_provider, Kimi):
            aksesa_client = llm.chat_provider.client
            headers = dict(aksesa_client._custom_headers)  # pyright: ignore[reportPrivateUsage]
            headers["User-Agent"] = f"{USER_AGENT}{ua_suffix}"
            aksesa_client._custom_headers = headers  # pyright: ignore[reportPrivateUsage]

    def _track_session_started(self, client: ClientInfo | None) -> None:
        if not isinstance(self._soul, KimiSoul):
            return

        from aksesa_cli.telemetry import track_session_started_once
